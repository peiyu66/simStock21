...
public class csvData {
    ...
    static func fetchStocksInfo (_ sId:[String]?=nil) -> StocksInfo {
        //在intents互傳NSManagedObject都會壞掉，可能是只能傳struct不能傳class？所以要在同個function內轉換。
        var info:StocksInfo = []
        let fetchRequest = Stock.fetchRequestStock(sId:sId)
        let context = coreData.shared.context
        let stocks:[Stock] = (try? context.fetch(fetchRequest)) ?? []
        for stock in stocks {
    ...
    static func getStocks (_ info:StocksInfo) -> [Stock] {
    ...
    static func csvTrans(_ stocks:StocksInfo, start:Date?=nil) -> String {
        var csv:String = ""
        ...
        for sId in stockIds {
            let fetchRequest = Stock.fetchRequestStock(sId:[sId])
            let matchedOne:[Stock] = (try? context.fetch(fetchRequest)) ?? []
            if let stock = matchedOne.first {
                stockObjects.append(stock)
            }
        }
        for stock in stockObjects {
            let fetchRequest = Trade.fetchRequest(stock: stock, start:(start ?? stock.dateStart), asc: true)
            let trades:[Trade] = (try? context.fetch(fetchRequest)) ?? []
            for trade in trades {
                ...
                let volume:String = String(format: "%.0f", trade.priceVolume)
                let volz:String = String(format: "%.2f", trade.tVolZ125)
    ...
    static func csvMonthlyRoi(_ stocks: [Stock], from:Date?=nil) -> String {
        var csv:String = ""
        ...
        for stock in stocks {
            ...
            let txt = csvData.shared.csvStockRoi(stock, from: tFrom)
    ...
    private func csvStockRoi(_ stock:Stock, from:Date) -> (header:String,body:String) {
        /*
