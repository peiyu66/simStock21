    ...
    var body: some View {
        GeometryReader { geo in
            VStack (alignment: .center) {
                ...
                if !list.doubleColumn { //!list.pageColumn(hClass)
            ...
            .onAppear {
                list.pageStock = self.stock
                if !list.pageColumn(hClass) && list.versionLast == "" && list.prefixStocks(prefix: prefix, group: (groupPrefixsOnly ? stock.group : nil)).count > 1 {
                    showPrefixMsg = true
                }
            }
        }
...
struct pageTools:View {
    ...
    var body: some View {
        HStack {
            ...
            .padding(.trailing, list.widthClass(hClass) == .compact ? 2 : 8)
...
//    }
    ...
    var totalSummaryText: some View {
        if let trade = stock.lastTrade(stock.context), trade.rollRounds > 0 {
            ...
            numberFormatter.numberStyle = .currency   //貨幣格式
            numberFormatter.maximumFractionDigits = 0
            let rollAmtProfit = Text((list.widthClass(hClass) == .compact ? "" : "累計") + "損益" + (numberFormatter.string(for: trade.rollAmtProfit) ?? "$0"))
            let rollAmtRoi = Text(" 年報酬" + (list.widthClass(hClass) == .compact ? "" : "率") + String(format:"%.1f%%",trade.rollAmtRoi/stock.years))
                .foregroundColor(stock.simInvestUser > 0 ? .orange : .primary)
            let rollDays = Text(" " + (list.widthClass(hClass) == .compact ? "" : "平均") + "週期" + String(format:"%.f天",trade.days))
                .foregroundColor(stock.simReversed ? .orange : .primary)
            return (rollAmtProfit + rollAmtRoi + rollDays)
        }
        return Text("尚無模擬交易")
    }
    ...
    var textAutoInvested: Text {
        if stock.simInvestAuto == 10 {
            ...
            if stock.simInvestExceed > 0 {
                return Text(String(format:"自動"))
                + Text(String(format:"%.0f+%.0f", stock.simInvestAuto, stock.simInvestExceed))
                    .foregroundColor(.red)
                + Text("次加碼")
            } else {
                return Text(String(format:"自動%.0f次加碼", stock.simInvestAuto))
            }
        } else {
            return Text("不自動加碼")
                .foregroundColor(.red)
    ...
    var body: some View {
        VStack (alignment: .trailing) {
            ...
            HStack {
                ...
                Text(stock.simMoneyBase > 0 ? String(format:"起始" + (list.widthClass(hClass) == .compact ? "" : "本金") + "%.f萬元",stock.simMoneyBase) : "")
                (textAutoInvested +
...
struct tradeCell: View {
    ...
    private func widthCG(_ CG:[CGFloat], width:CGFloat?=nil, max:CGFloat?=100) -> CGFloat {
        ...
        let cg = w * list.widthCG(hClass, CG: CG) / 100
//        NSLog("\(list.widthClass(hClass)) \(w) \(CG) \(cg)")
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack {
                ...
                .frame(width: list.widthCG(hClass, CG:[16,20]), alignment: .center)
                //== 2日期,3單價 ==
                ...
                if trade.simQtyInventory > 0 || trade.simQtySell > 0 {
                    ...
                    if list.widthClass(hClass) > .compact || trade.simQtySell > 0 {
                ...
                Group {
                    if trade.simRuleInvest == "A" { //trade.invested = simInvestByUser + simInvestAdded
                        ...
                        Text((trade.invested > 0 ? "已加碼" + String(format:"(%.f)",trade.simInvestTimes - 1) : "+   ") + (trade.simInvestByUser > 0 ? "+" : (trade.simInvestByUser < 0 ? "-" : " ")))
                    }
                }
            ...
            if list.selected == trade.date {
                ...
                HStack {
                    ...
                    if let p10Date = stock.p10Date, trade.date == p10Date {
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            if list.widthClass(hClass) > .compact || (L.count <= 2 && H.count <= 2){
                                HStack {
                ...
                //=== 擴充技術數值 ===
                if list.widthClass(hClass) > .widePhone {
                    HStack {
                        Text("").frame(width: 20.0, alignment: .center)
                        Group {
                            VStack(alignment: .trailing,spacing: 2) {
