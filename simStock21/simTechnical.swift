    ...
    func downloadStocks(doItNow:Bool = false) { //每10天下載股票代號簡稱清單
        if doItNow {
            ...
            if days > 10 {    //10天更新一次
                ...
                simLog.addLog("stocks list 上次：\(twDateTime.stringFromDate(timeStocksDownloaded,format: "yyyy/MM/dd HH:mm:ss")), next in \(String(format:"%.1f",10 - days)) days")
            }
        } else {
            twseDailyMI()
        }
    ...
    func reviseCompanyInfo(_ stocks:[Stock]) {
        ...
        if let timeCompanyInfoUpdated = defaults.object(forKey: "timeCompanyInfoUpdated") as? Date {
            ...
            if days > 30 {
                ...
                simLog.addLog("companyInfo 上次：\(twDateTime.stringFromDate(timeCompanyInfoUpdated,format: "yyyy/MM/dd HH:mm:ss")), next in \(String(format:"%.1f",30 - days)) days")
            }
        } else {
            toBeRevised = true
        }
                                    ...
                                    if (!twDateTime.isDateInToday(dt1)) && Date() > time0905 {
                                        ...
                                        if close > 0 {
                                            ...
                                            if (dt1 > trade.dateTime || trade.priceClose != close) && trade.dataSource != "TWSE" {
                                                ...
                                                trade.priceVolume = yNumber(yColumn[4])
...
//        }
    ...
    func twseRequest(stock:Stock, dateStart:Date, stockGroup:DispatchGroup) {
        ...
        let task = URLSession.shared.dataTask(with: request, completionHandler: {(data, response, error) in
            do {
                guard let jsonData = data else { throw simTechnical.requestError.error(msg:"no data") }
                guard let jroot = try JSONSerialization.jsonObject(with: jsonData, options: .allowFragments) as? [String:Any] else {throw simTechnical.requestError.error(msg: "invalid jroot") }
                guard let stat = jroot["stat"] as? String else {throw simTechnical.requestError.error(msg:"no rtmessage") }
                if stat != "OK" {
                    throw simTechnical.requestError.error(msg:"stat is not OK")
                }
                guard let jdata = jroot["data"] as? [[String]] else {throw simTechnical.requestError.warning(msg:"沒有交易資料？")}
                    ...
                    if let dt0 = twDateTime.dateFromString(element[0]) {
                        if let dt = twDateTime.calendar.date(byAdding: .year, value: 1911, to: dt0) {
                            if let close = Double(element[6].replacingOccurrences(of: ",", with: "")), close > 0 {
                                ...
                                trade.priceVolume = (Double(element[1].replacingOccurrences(of: ",", with: "")) ?? 0) / 1000
                                trade.dataSource   = "TWSE"
                                trade.tUpdated  = false
                                count += 1
                                try? context.save()
            ...
            } catch simTechnical.requestError.warning(let msg) {
                simLog.addLog("\(stock.sId)\(stock.sName) TWSE \(twDateTime.stringFromDate(dateStart)) \(msg)")
            } catch simTechnical.requestError.error(let msg) {
                simLog.addLog("\(stock.sId)\(stock.sName) TWSE \(twDateTime.stringFromDate(dateStart)) \(msg)")
            } catch {
    ...
    func twseCsv(stock:Stock, dateStart:Date, stockGroup:DispatchGroup) {
        ...
        let task = URLSession.shared.dataTask(with: request, completionHandler: {(data, response, error) in
            do {
                guard let d = data else {throw technical.requestError.error(msg:"data is nil") }
                let big5 = CFStringConvertEncodingToNSStringEncoding(CFStringEncoding(CFStringEncodings.dosChineseTrad.rawValue))
                guard let downloadedData = String(data:d, encoding:String.Encoding(rawValue: big5)) else { throw technical.requestError.error(msg:"invalid data") }
...
//            }
            ...
            trade.tVolMax9 = trade.priceVolume
            trade.tVolMin9 = trade.priceVolume
            for t in trades[d9.thisIndex...index] {
                ...
                if t.priceVolume > trade.tVolMax9 {
                    trade.tVolMax9 = t.priceVolume
                }
                if t.priceVolume < trade.tVolMin9 {
                    trade.tVolMin9 = t.priceVolume
                }
            }
            ...
            trade.tVolZ125 = standardDeviationZ("priceVolume", dIndex:d125)
            trade.tVolZ250 = standardDeviationZ("priceVolume", dIndex:d250)
...
//            let sRoi02 = trade.simUnitRoi > (trade.grade <= .weak ? 1.5 : 2.5)
            ...
            let sBase5 = wantS >= topWantS && sRoi00
            let sBase4 = wantS >= (topWantS - 1) && sRoi02
            let sBase3 = wantS >= (topWantS - 2) && (sRoi03 || (sRoi00 && trade.simDays > 75))
            let sBase2 = wantS >= (topWantS - 3) && (sRoi18 || sRoi13 || sRoi09)
            ...
            let sCut = wantS >= (trade.grade >= .none && trade.simDays < 400 ? 1 : 2) && (cut1 || cut2) && noInvested60
